...
const HomeScreen = () => {
      ...
      <Col key={item._id} xs={12} sm={6} md={4} lg={3} className='p-3'>
        ...
        >
          ...
          <BootstrapCard.Body>
            ...
            <h5 className='fw-semibold text-primary d-flex align-items-center justify-content-center gap-2'>
              {truncateText(item.title || item.courseTitle, 25)}
            </h5>
            <h5>
            {type === 'courses' && (
                <span className={`badge ${getDifficultyBadgeColor(item.difficulty)}`}>
                {item.difficulty ? item.difficuly : 'No course difficulty is defined'}
              </span>
              )}
            </h5>
            {/* Hide Description and Category for Courses */}
            {type !== 'courses' && (
              <>
                <p className='text-muted small'>
                  {truncateText(item.description || item.company?.name || 'No details provided', 100)}
                </p>
  ...
  return (
    ...
    >
        ...
        >
          <div className="text-container" style={{ maxWidth: "45%" }}>
            ...
            <p className="fs-5 mt-3 text-black">
              Our platform connects your skills to internships and hiring
              opportunities enabling personal and professional growth.
            </p>
            <Button
              variant="light"
              size="lg"
      ...
      {activeJobs.length > 0 &&
        renderSection("Featured Jobs", activeJobs.slice(0, 8), "job-info")}
        ...
        <div className="d-flex justify-content-center gap-3 mb-4 flex-wrap">
          {categories.map((category) => (
            <Button
              key={category}
              variant={
                activeCategory === category ? "dark" : "outline-secondary"
              }
      ...
      <Container fluid className="my-5">
        ...
        >
          <h3 className="fw-bold mb-3">
            Want to post jobs and hire skilled workforce?
          </h3>
